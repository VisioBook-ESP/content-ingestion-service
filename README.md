...
Service de traitement et d'ingestion de documents pour la plateforme ESP (VisioBook). Ce microservice extrait le texte, les metadonnees et decoupe les documents en chunks pour l'indexation et la recherche semantique.

## Fonctionnalites

- **Extraction de texte** : PDF, DOCX, HTML, TXT, images (OCR)
- **OCR** : Reconnaissance optique de caracteres avec Tesseract (francais/anglais)
- **Nettoyage de texte** : Normalisation, suppression headers/footers, correction encodage
- **Chunking** : Decoupage intelligent avec overlap configurable
- **Detection de langue** : FR, EN, ES, DE, IT
- **Extraction de metadonnees** : Titre, auteur, nombre de pages, etc.
...
│   ├── extract.py        # Extraction de metadonnees
...
|---------|----------|-------------|
| `POST` | `/api/v1/ingest/` | Demarrer une ingestion asynchrone |
| `GET` | `/api/v1/ingest/status/{job_id}` | Statut d'un job |
| `POST` | `/api/v1/ingest/cancel/{job_id}` | Annuler un job |
| `POST` | `/api/v1/extract/metadata` | Extraire les metadonnees d'un fichier |
| `POST` | `/api/v1/preprocess/clean` | Nettoyer du texte |
| `POST` | `/api/v1/preprocess/chunk` | Decouper en chunks |
| `GET` | `/api/v1/health` | Health check |

### Exemple d'ingestion

```bash
# Demarrer une ingestion
...
# Rebuild apres modifications
docker-compose up -d --build
```

### Services inclus

- **content-ingestion-service** : API principale (port 8090)
- **db** : PostgreSQL 14 (port 5433)
- **redis** : Redis 7 (port 6380)

## Documentation API

Une fois le service demarre, acceder a :
...
| `DATABASE_SERVICE_URL` | URL du service de base de donnees | - |
